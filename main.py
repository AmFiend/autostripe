...
# Fix encoding for Windows terminal to support Unicode characters
...
def banner():
    ...
    print(f"{Fore.CYAN}{Style.BRIGHT}       STRIPE AUTH BY {author}")
    print(f"{Fore.CYAN}{Style.BRIGHT}" + "="*50)
...
async def process_stripe_card(base_url, card_data, proxy_url=None, auth_mode=1, shared_email=None, shared_password=None):
    """
    Enhanced Stripe Auth logic extracted from stripe auth test
    """
    ua = UserAgent()
    ...
    try:
        ...
        # Security Note: Built by @mumiru_bro
                ...
                try:
                    ...
                    if 'success' in text:
                        ...
                        if js.get('success'):
                            ...
                            if status == 'succeeded': return True, f"Approved (Status: succeeded){branding}"
...
async def main():
    while True:
        ...
        site = input(f"{Fore.YELLOW}Enter Site URL: {Style.RESET_ALL}").strip()
        if not site:
            print(f"{Fore.RED}Invalid Site URL")
            time.sleep(2)
            continue
        site = normalize_url(site)
            
        if choice == '1':
            cc = input(f"{Fore.YELLOW}Enter CC (NUM|MM|YY|CVV): {Style.RESET_ALL}").strip()
            card_data = parse_card_data(cc)
            if not card_data:
                print(f"{Fore.RED}Invalid Card Format")
                time.sleep(2)
                continue
                
            print(f"{Fore.CYAN}[*] Checking Card...")
            is_approved, msg = await process_stripe_card(site, card_data)
            
            if is_approved:
                print(f"{Fore.GREEN}[+] {cc} -> {msg}")
            else:
                print(f"{Fore.RED}[-] {cc} -> {msg}")
            ...
            print(f"{Fore.CYAN}[*] Loaded {len(cards)} cards. Starting check...")
            
            promo_msg = "make sure to join my offical chaneel for free stuff\n@mumiru_backup\n" + "-"*50 + "\n"
            for f_name in ["approved.txt", "declined.txt"]:
                if not os.path.exists(f_name):
                    with open(f_name, "w", encoding='utf-8') as f: f.write(promo_msg)
                else:
                    with open(f_name, "r", encoding='utf-8') as f:
                        content = f.read()
                    if "@mumiru_backup" not in content:
                        with open(f_name, "w", encoding='utf-8') as f: f.write(promo_msg + content)
            ...
            for cc in cards:
                ...
                card_data = parse_card_data(cc)
                if not card_data:
                    print(f"{Fore.RED}[-] {cc} -> Invalid Format")
                    continue
                ...
                if is_approved:
                    print(f"{Fore.GREEN}[+] {cc} -> {msg}")
                    with open("approved.txt", "a") as f: f.write(f"{cc} -> {msg}\n")
                    with open("declined.txt", "a") as f: f.write(f"{cc} -> {msg}\n")
            

            print(f"\n{Fore.YELLOW}Check complete. Results saved with author credits.")
